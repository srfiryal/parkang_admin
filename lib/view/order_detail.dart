...
class _OrderDetailState extends State<OrderDetail> {
  ...
  Widget _buildOrderStatus() {
    return Row(
      ...
      children: [
        ...
        Text(widget.orderModel.status, style: GoogleFonts.poppins(fontWeight: FontWeight.bold, color: SharedCode.getOrderStatusColor(context, widget. orderModel.status))),
      ],
    );
  }
    ...
    return Column(
      ...
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text(widget.orderModel.buyersName, style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
            Text(widget.orderModel.buyersPhone)
          ],
        ),
  ...
  Widget _buildOrderItemList() {
    return ListView.builder(
        ...
        itemBuilder: (context, index) {
          return _buildOrderItemLayout(widget.orderModel.items[index], index, widget.orderModel.items.length);
        }
    );
  }
  
  Widget _buildOrderItemLayout(OrderItemModel item, int index, int length) {
    return Column(
      children: [
        Padding(
          ...
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Image.network(item.productImageUrl, width: 70.0, height: 70.0, fit: BoxFit.cover),
                  ...
                  children: [
                    ...
                    Row(
                        ...
                        children: [
                          ...
                          Text(SharedCode.rupiahFormat.format(item.pricePerItem)),
                        ]
                    ),
                    const SizedBox(height: 7.0),
                    Align(alignment: Alignment.centerRight, child: Text(SharedCode.rupiahFormat.format(item.totalPrice), style: GoogleFonts.poppins(color: Theme.of(context).primaryColor, fontWeight: FontWeight.w500))),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildShipmentInfo() {
    return Column(children: [
      _buildTextLabel('Shipment Method', widget.orderModel.shipmentMethod),
  ...
  Widget _buildPricing() {
    return Column(children: [
      _buildTextLabel('Payment Method', widget.orderModel.paymentMethod),
      const SizedBox(height: 10.0),
      _buildPricingLabel('Order Summary', widget.orderModel.orderSummary, false),
      const SizedBox(height: 2.0),
      _buildPricingLabel('Shipping Fee', widget.orderModel.shippingFee, false),
      const SizedBox(height: 2.0),
